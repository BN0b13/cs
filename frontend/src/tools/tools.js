...
export default class Tools {
  ...
  passwordRegex = /^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*\W)(?!.* ).{8,30}$/;
  passwordMinLength = 8;
  ...
  integerInputValidation = (e, setInput) => {
    const removeCurrency = e.replace('$', '');
    const removePeriod = removeCurrency.replace('.', '');
    if(removePeriod === '') {
      return setInput(removePeriod);
    }

    if(this.integerRegex.test(removePeriod)) {
        return setInput(parseInt(removePeriod));
    } else {
        return
    }
  }
